...
func TestExternalIdpImportReturnsRotatedRefreshTokenWhenPersistFails(t *testing.T) {
	...
	// SQLite is primary storage; close it so Save falls back to JSON, then
	// sabotage the JSON path (config.json may already have been archived).
	config.CloseStore()
	if err := os.WriteFile(cfgPath, []byte("{}"), 0o600); err != nil && !os.IsExist(err) {
		// If a directory somehow exists, remove it first.
